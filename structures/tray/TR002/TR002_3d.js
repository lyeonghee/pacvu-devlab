// TR002 EB Tray Box 3D assembly preview.
(function(root){
  'use strict';
  const T=.22,hinges=[];
  ...
  function install(){
    if(document.getElementById('tr002-3d-btn'))return;
    const toolbar=document.querySelector('.toolbar');if(!toolbar)return;
    const button=el('button','btn m001-3d-button','3D MOCKUP');button.id='tr002-3d-btn';toolbar.append(button);
    const viewer=root.PacVu3DViewer.createModal({id:'tr002-3d-modal',badge:'TR002 · EB Tray Box'});
    ...
    document.addEventListener('input',event=>{
      if(!['baseW','baseD','panelH','tr002FrontBackHoleCount','tr002FrontBackHoleWidth','tr002FrontBackHoleHeight','tr002LeftRightHoleCount','tr002LeftRightHoleDiameter'].includes(event.target?.id)||!modal.classList.contains('open'))return;
  ...
  function sync(){
    const button=document.getElementById('tr002-3d-btn');if(!button)return;
    const active=typeof selectedBoxMeta!=='undefined'&&selectedBoxMeta.engineKey==='tr002';
  ...
  function materials(){
    if(!boardMaterials){
      ...
      boardMaterials[2].name='TR002 folded paper edge';
    }
  ...
  function outerFace(elements,tr){
    const raw=elements.flatMap(element=>TR002_sourceElementSegments(element,tr));
    const nodes=[];
    const findNode=(x,y)=>{
      let best=-1,distance=1.1;
      nodes.forEach((node,index)=>{const next=Math.hypot(node.x-x,node.y-y);if(next<distance){best=index;distance=next;}});
      if(best>=0)return best;
      nodes.push({x,y,links:[]});return nodes.length-1;
    };
    raw.forEach(segment=>{
      const a=findNode(segment[0],segment[1]),b=findNode(segment[2],segment[3]);
      if(a===b||nodes[a].links.includes(b))return;
      nodes[a].links.push(b);nodes[b].links.push(a);
    });
    nodes.forEach((node,index)=>node.links.sort((a,b)=>
      Math.atan2(nodes[a].y-node.y,nodes[a].x-node.x)-Math.atan2(nodes[b].y-node.y,nodes[b].x-node.x)
    ));
    const visited=new Set(),faces=[];
    nodes.forEach((node,a)=>node.links.forEach(b=>{
      const start=`${a}:${b}`;if(visited.has(start))return;
      const face=[];let from=a,to=b,closed=false;
      for(let guard=0;guard<raw.length*4;guard+=1){
        const key=`${from}:${to}`;if(visited.has(key)){closed=key===start;break;}
        visited.add(key);face.push({x:nodes[from].x,y:nodes[from].y});
        const links=nodes[to].links,index=links.indexOf(from);if(index<0||links.length<2)break;
        const next=links[(index-1+links.length)%links.length];from=to;to=next;
        if(`${from}:${to}`===start){closed=true;break;}
      }
      if(closed&&face.length>2)faces.push(face);
    }));
    const area=points=>points.reduce((sum,point,index)=>{const next=points[(index+1)%points.length];return sum+point.x*next.y-next.x*point.y;},0)/2;
    return faces.sort((a,b)=>Math.abs(area(b))-Math.abs(area(a)))[0]||[];
  }
  ...
  function regionMesh(name,outline,mask,holes,cutouts=[]){
    ...
    if(polygon.length<3){const empty=new THREE.Group();empty.name=`${name}-empty`;return empty;}
    polygon.forEach((point,index)=>index?shape.lineTo(point.x,-point.y):shape.moveTo(point.x,-point.y));shape.closePath();
    holes.filter(hole=>hole.cx>=mask[0].x&&hole.cx<=mask[2].x&&hole.cy>=mask[0].y&&hole.cy<=mask[2].y).forEach(hole=>{
      if(hole.shape==='circle'){
        const path=new THREE.Path();path.absarc(hole.cx,-hole.cy,hole.width/2,0,Math.PI*2,false);shape.holes.push(path);
      }else{
        shape.holes.push(roundedRectHole(THREE,{x1:hole.cx-hole.width/2,x2:hole.cx+hole.width/2,y1:hole.cy-hole.height/2,y2:hole.cy+hole.height/2,r:hole.height/2}));
      }
    });
  ...
  function build(reset){
    const cfg=typeof getCfgTR002==='function'?getCfgTR002():{W:200,D:280,H:100,frontBackHoleCount:1,frontBackHoleWidth:30,frontBackHoleHeight:16,leftRightHoleCount:1,leftRightHoleDiameter:15},layout=root.TR002_getLayout(cfg.W,cfg.D,cfg.H,cfg),p=layout.params;
    ...
    const source=TR002_loadReferenceSvg(),tr=TR002_transformer(layout.spec);
    const outline=outerFace(source.cutElements.concat(source.foldElements),tr);if(outline.length<3)throw new Error('TR002 outer cutPath is unavailable');
    const holes=layout.holes||[],sx=cfg.W/200,sy=cfg.H/100,sl=layout.spec.dustFlap/50,mid=(p.x1+p.x2)/2;
  ...
  function downloadPng(){const canvas=renderer?.domElement;if(!canvas)return;canvas.toBlob(blob=>{if(!blob)return;const link=document.createElement('a');link.href=URL.createObjectURL(blob);link.download=`TR002_3D_${Math.round(Number(range.value))}.png`;link.click();setTimeout(()=>URL.revokeObjectURL(link.href),1000);},'image/png');}
  ...
  root.TR002_3D_syncAvailability=sync;
